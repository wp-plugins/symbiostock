...
class ssHelper {
	function ssHelper() {
		...
		$this->ss_watermark_loc = $this->ss_rootdir . 'assets/watermark.png';
	...
	function error_notice_noimagick() {
		?>
		<div class="error">
			<p><?php print __('Symbiostock requires the Imagick PHP extension to run. Please install or enable Imagick.','ss'); ?></p>
		</div>
		<?php
	}
	...
	function get_all_products() {
		if (isset($this->product_list)) return $this->product_list;
		...
		foreach ($c as $p) $this->product_list[] = $p[0];
