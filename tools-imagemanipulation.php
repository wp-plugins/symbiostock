...
function ss_resize_sold_image($fileurl,$width,$height,$noupscale=1) {
	...
	$image = new Imagick();
	if (method_exists($image,'setResolution')) $image->setResolution(3000,3000);
	if (!$image->readImage($fileurl)) return false;
	if (!method_exists($image,'setResolution') && method_exists($image,'setImageResolution')) $image->setImageResolution(3000,3000);
...
function ss_make_watermark($fileurl) {
	...
	$image = new Imagick();
	if (method_exists($image,'setResolution')) $image->setResolution(300,300);
	if (!$image->readImage($fileurl)) return false;
	if (!method_exists($image,'setResolution') && method_exists($image,'setImageResolution')) $image->setImageResolution(300,300);
 
	$watermark = new Imagick();
	if (method_exists($watermark,'setResolution')) $watermark->setResolution(300,300);
	if (!$watermark->readImage($GLOBALS['currSS']->ss_watermark_loc)) return false;
	if (!method_exists($watermark,'setResolution') && method_exists($watermark,'setImageResolution')) $watermark->setImageResolution(300,300);
	...
	if (class_exists('ImagickDraw')) {
		...
		if (method_exists($draw,'setResolution')) $draw->setResolution(300,300);
