...
function ss_clear_stored_image($postid) {
	...
	if (file_exists($fileloc) && !is_dir($fileloc)) unlink($fileloc);
}

// Delete product variations when license is deleted
add_action( 'pre_delete_term', 'ss_clear_product_variations',10,2);
function ss_clear_product_variations($termid,$taxonomy) {
	@ignore_user_abort(1);
	if ($taxonomy == 'pa_license') {
		global $wpdb;
		$term_obj = get_term($termid, $taxonomy);
		$c = $wpdb->get_results("SELECT post_id FROM $wpdb->postmeta WHERE meta_key = 'attribute_pa_license' and meta_value = '".$term_obj->slug."'", ARRAY_N);
		if ($c) {
			foreach ($c as $p) {
				$currvariation = $p[0];
...
add_filter( 'woocommerce_downloadable_file_exists', 'ss_returntrue');
function ss_returntrue() {
	return true;
}
