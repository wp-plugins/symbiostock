...
function ss_write_image_metadata($postid) {
		...
		if (isset($_POST['_ss_update_metadata'])) {
			...
			$tester = get_post_meta($postid, '_ss_update_metadata', true);
		}

		if ($tester == '1') $parse = 1;
		elseif (!$tester && get_option('ss_default_update_metadata')) $parse = 1;

		if (!$parse) return;

		$filename = $GLOBALS['currSS']->ss_media_dir.get_post_meta($currpost->ID, 'ss_media_filename', true);

		if (ss_get_mime_type($filename) != 'image/jpeg') return;

		$title = $currpost->post_title;
		$description = $currpost->post_content;
		if (trim($currpost->post_excerpt)) $description = $currpost->post_excerpt;

		$currproduct = wc_get_product($currpost->ID);
		$tags = explode(', ',$currproduct->get_tags());
