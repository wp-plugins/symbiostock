	...
	if (in_array('shell_exec', $disabled)) {
		...
		if(is_array($info)) {
	        ...
	        foreach (array_keys($iptc) as $s) {
				if ($s=='2#105' && $iptc[$s][0]) $object->Headline = $iptc[$s][0];
				if ($s=='2#005' && $iptc[$s][0]) $object->Title = $iptc[$s][0];
				if ($s=='2#120' && $iptc[$s][0]) $object->Description = $iptc[$s][0];

				if ($s=='2#025') {
	            	$c = count ($iptc[$s]);
	            	for ($i=0; $i <$c; $i++) {
	               		$object->Keywords[] = $iptc[$s][$i];
	            	}
				}
	        }
    	}
        ...
        if ($results && is_string($results)) {
			...
			$object = $meta_object[0];
		}
	}

	if ($object) {
       	for ($i=0; $i <count($object->Keywords); $i++) {
      		$object->Keywords[$i] = strval($object->Keywords[$i]);
      	}
		return $object;
	}
 	else return 0;
...
function ss_write_image_metadata($postid) {
		...
		if ((ss_get_mime_type($filename) != 'image/jpeg') && (ss_get_mime_type($filename) != 'image/tiff')) return;
