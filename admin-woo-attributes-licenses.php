...
<?php
if (get_option('ss_v_enabled')) {
	$xtratxt = " Not applicable to videos.";
?>
	<tr class="form-field">
...
?>
	...
	<th scope="row" valign="top"><label for="term_meta[ss_license_output_vector]"><?php _e( 'Sell unaltered', 'ss' ); ?></label></th>
		...
		<p class="description"><?php _e( 'Download raw file. If not enabled, item will be converted to raster for this license. Useful for vector sales.'.$xtratxt,'ss' ); ?></p>
		</td>
	</tr>
	<tr class="form-field">
	...
	<th scope="row" valign="top"><label for="term_meta[ss_license_max_x]"><?php _e( 'Maximum width', 'ss' ); ?></label></th>
		<td>
			...
			<p class="description"><?php _e( 'The maximum width in pixels this license provides for. Not applicable if "sell as vector" is checked. 0 for max.'.$xtratxt,'ss' ); ?></p>
		</td>
	</tr>
	<tr class="form-field">
	<th scope="row" valign="top"><label for="term_meta[ss_license_max_y]"><?php _e( 'Maximum height', 'ss' ); ?></label></th>
		<td>
			...
			<p class="description"><?php _e( 'The maximum height in pixels this license provides for. Not applicable if "sell as vector" is checked. 0 for max.'.$xtratxt,'ss' ); ?></p>
		</td>
	</tr>
<?php
}
...
function ss_save_licenses_custom_meta( $term_id ) {
	...
	if ( isset( $_POST['term_meta'] ) ) {
		...
		$term_meta = ss_get_option( "taxonomy_$t_id" );
		...
		ss_update_option( "taxonomy_$t_id", $term_meta );
			...
			if ( isset ( $_POST['term_meta'][$key] ) ) {
				...
				if ($key == 'ss_license_update_prices') {
					...
					foreach ( $myposts as $currvariation ) {
						if (ss_get_post_meta( $currvariation, 'attribute_pa_license', true ) == $_POST['slug']) {
						    ss_update_post_meta($currvariation, '_price', sanitize_text_field($_POST['term_meta']['ss_default_license_price']));
						    ss_update_post_meta($currvariation, '_regular_price', sanitize_text_field($_POST['term_meta']['ss_default_license_price']));
							$GLOBALS['currSS']->ss_refresh_product(wp_get_post_parent_id($currvariation));
						}
					}
				}
				if ($key == 'ss_license_update_licenses') {
					$myp = ss_get_all_products();
					foreach ( $myp as $currproduct ) {
						$mediatypes = array('raster','vector','video');
						foreach ($mediatypes as $mediatype) {
							if (ss_get_post_meta( $currproduct, '_ss_mediatype', true ) == $mediatype) {
								$myposts = ss_get_product_vars($currproduct);
								$found = 0;
								foreach ( $myposts as $currvariation ) {
									if (ss_get_post_meta( $currvariation, 'attribute_pa_license', true ) == $_POST['slug']) {
										if (!$_POST['term_meta']['ss_license_'.$mediatype]) {
											wp_delete_post($currvariation);
											$GLOBALS['currSS']->ss_refresh_product($currproduct);
										}
										$found = 1;
									}
								}
								if (!$found && $_POST['term_meta']['ss_license_'.$mediatype]) {
									...
									ss_add_license_as_variation($currproduct,$licensetoadd);
								}
							}
						}
						ss_add_all_licenses_to_product($currproduct);
					}
				}
			}
		}
