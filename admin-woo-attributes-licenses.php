		...
		foreach ( $cat_keys as $key ) {
			if ( isset ( $_POST['term_meta'][$key] ) ) {
				...
				if ($key == 'ss_license_update_prices') {
					$args = array( 'post_type' => 'product_variation', 'posts_per_page'=> 99999999,'fields' => 'ids');
					$myposts = get_posts($args);
					foreach ( $myposts as $currvariation ) {
						if (get_post_meta( $currvariation, 'attribute_pa_license', true ) == $_POST['slug']) {
						    update_post_meta($currvariation, '_price', sanitize_text_field($_POST['term_meta']['ss_default_license_price']));
						    update_post_meta($currvariation, '_regular_price', sanitize_text_field($_POST['term_meta']['ss_default_license_price']));
						}
					}
				}
				if ($key == 'ss_license_update_licenses') {
					$myp = $GLOBALS['currSS']->get_all_products();
					foreach ( $myp as $currproduct ) {
						$mediatypes = array('raster','vector','video');
						foreach ($mediatypes as $mediatype) {
							if (get_post_meta( $currproduct, '_ss_mediatype', true ) == $mediatype) {
								$args = array( 'post_type' => 'product_variation', 'post_parent'=> $currproduct, 'posts_per_page'=> 99999999,'fields' => 'ids');
								$myposts = get_posts($args);
								$found = 0;
								foreach ( $myposts as $currvariation ) {
									if (get_post_meta( $currvariation, 'attribute_pa_license', true ) == $_POST['slug']) {
