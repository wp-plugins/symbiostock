...
2) Click 'Publish'.<br>
<br>
<b>New products will be created once the scheduled maintenance process parses your uploaded file(s).</b><br><br>Uploads over <b>8MB</b> in size may not work using this web form. It is recommended you use FTP for most media uploads. FTP details are located in  Symbiostock 'Settings'.</div></div></p>
<?php 
...
add_action('woocommerce_product_bulk_edit_end','ss_bulk_edit_fields');
function ss_bulk_edit_fields() {
?>
</div>
<label>
				<div class="title"><i>Title</i></div></label>
				<span class="input-text-wrap"><input type="text" name="ss_bulk_title" value="" /></span>

						<label>
				<div class="title"><i>Content/Description</i></div></label>
				<textarea rows="2" name="ss_bulk_content" class="tax_input_product_tag" style="width:100%"></textarea>
<?php
	$fields[] = '_ss_editorial';
	$fields[] = '_ss_exclusive';
	$fields[] = '_ss_modelrelease';
	$fields[] = '_ss_propertyrelease';
	$titles[] = 'Editorial';
	$titles[] = 'Exclusive';
	$titles[] = 'Model Release';
	$titles[] = 'Property Release';
	for ($i=0;$i<count($fields);$i++) {
?>
		<label class='inline-edit-tags'>
			<div class="title"><i><?php print __( $titles[$i], 'ss' ); ?></i></div></label>
				<span class="input-text-wrap">
					<input type="radio" name="<?php print $fields[$i]; ?>" value="1"> Yes &nbsp;
					<input type="radio" name="<?php print $fields[$i]; ?>" value="2"> No &nbsp;
					<input type="radio" name="<?php print $fields[$i]; ?>" value="" checked> No Change
			</span>
<?php
	}
	if ($GLOBALS['currSS']->simplify()) {
?>
<style> div#woocommerce-fields-bulk {display:none;} </style>
<?php
	}
}

add_action('woocommerce_product_bulk_edit_save','ss_bulk_save_fields');
function ss_bulk_save_fields($product) {
	$fields[] = '_ss_editorial';
	$fields[] = '_ss_exclusive';
	$fields[] = '_ss_modelrelease';
	$fields[] = '_ss_propertyrelease';
	foreach ($fields as $field) {
		if ($_REQUEST[$field]) {
			if ($_REQUEST[$field] == 1) $t = 'on';
			else $t = '';
			update_post_meta( $product->id, $field, $t);
		}
	}
	if (trim($_REQUEST['ss_bulk_title']) || trim($_REQUEST['ss_bulk_content'])) {
		$my_post['ID'] = $product->id;

		if ( ! wp_is_post_revision($my_post['ID']) ){
			// remove and add is to prevent looping (WP bug)
			remove_action('woocommerce_product_bulk_edit_save', 'ss_bulk_save_fields');

			if ($t = sanitize_text_field(trim($_REQUEST['ss_bulk_title']))) $my_post['post_title'] = $t;
			if ($t = sanitize_text_field(trim($_REQUEST['ss_bulk_content']))) $my_post['post_content'] = $t;
